...
from app.lib.db.schemas import UserSchema, UserCreateSchema, OrderRequestSchema, OrderCreateSchema
...
async def create_order(db_obj: OrderRequestSchema, db: AsyncSession = Depends(get_session)):
    ...
    if not user:
        ...
        user_data = UserCreateSchema(**db_obj.model_dump(exclude={"service_id", "status"}))
